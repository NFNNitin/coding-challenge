...
def yield_data():
    path = r"C:\Users\nitin\Downloads\code-challenge-template\yld_data\US_corn_grain_yield.txt"
    ...
    dataframe.to_sql("yield", con=engine)
...
def calculate_stats(path):
    ...
    tp = 'mysql+pymysql://{}:{}@{}/{}'.format(user, password, host, database_name)
    engine = create_engine(tp, echo=False)
    dataframe.to_sql("weather_stats", con=engine)
...
if __name__ == "__main__":
    path = r"C:\Users\nitin\Downloads\code-challenge-template\wx_data\USC00110072.txt"
    read_data(path)
    yield_data()
